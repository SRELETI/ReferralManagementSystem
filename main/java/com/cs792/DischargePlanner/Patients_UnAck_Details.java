...
public class Patients_UnAck_Details extends Activity {
	...
	private String completedByStr;

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.activity_patients__un_ack__details);

		Intent intent = getIntent();
		intent_status = intent.getStringExtra("status");
		intent_completedBy = intent.getStringExtra("completedBy");
		...
		completedBy = (TextView) findViewById(R.id.Patient_Detailed_status);
		date_view = (TextView) findViewById(R.id.Patient_Detailed_SubmitDate);

		new HeavyWorker(Patients_UnAck_Details.this).execute();
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		getMenuInflater().inflate(R.menu.menu_patients__un_ack__details, menu);
		return true;
	}
	...
	private class HeavyWorker extends AsyncTask<Void, Void, Wrapper_unAssigned> {
		...
		@Override
		protected Wrapper_unAssigned doInBackground(Void... params) {
			...
			Wrapper_unAssigned wrapper_unAssigned = new Wrapper_unAssigned();

			String[] names = intent_name.split(",");
			if(names.length!=2) {
				return wrapper_unAssigned;
			}
			...
			completedByStr = cursor_referralData.getString(cursor_referralData.getColumnIndex(PatientReferralAdapater.REFERRAL_COMPLETED_BY));
			...
			wrapper_unAssigned.head = headings;
			wrapper_unAssigned.full = full_details;
			return  wrapper_unAssigned;
		}

		@Override
		protected void onPostExecute(Wrapper_unAssigned wrapper_unAssigned) {
			super.onPostExecute(wrapper_unAssigned);
			if(progressDialog.isShowing()) {
				progressDialog.dismiss();
			}
			progressBar.getProgressDrawable().setColorFilter(Color.RED, PorterDuff.Mode.SRC_IN);
			photo.setImageResource(R.drawable.user);
			completedBy.setText("Submitted By: "+completedByStr);
			name.setText(firstName+" "+lastName);
			date_view.setText("Submitted On: "+discharge_date);
			detailsAdapter = new PatientDetailedViewCustomAdapter(Patients_UnAck_Details.this,wrapper_unAssigned.head,wrapper_unAssigned.full);
